...
namespace vez
{
    DescriptorPool::DescriptorPool(DescriptorSetLayout* layout) : _layout(layout)
    {
        ...
        {
            descriptorTypeCounts[binding.second.descriptorType] += binding.second.descriptorCount;
        }
    ...
    VkDescriptorSet DescriptorPool::AllocateDescriptorSet()
    {
        ...
        while (true)
        {
            ...
            else if (_allocatedSets[_currentAllocationPoolIndex] < _maxSetsPerPool)
                break;
